...
async function getSessionData(session: string, token: string) {
  ...
  const data = await fetch(
    `https://api-on.tuyensinh247.com/admin/v1/exercise-sheet/session/${session}`,
    {
      headers: {
        "Content-Type": "application/json",
...
async function getExercises(sessionData: any) {
  const $ = cheerio.load(
  ...
  QnAs.forEach((questionsAndAnswer: any) => {
    var answerA: string =
      questionsAndAnswer.question.answers[0].text[0].content;
    var answerB: string =
      questionsAndAnswer.question.answers[1].text[0].content;
    var answerC: string =
      questionsAndAnswer.question.answers[2].text[0].content;
    var answerD: string =
      questionsAndAnswer.question.answers[3].text[0].content;
    // console.log(answerA);
    ...
    $selected.append(
      `<strong>${index++}. </strong>${questionsAndAnswer.question.content[0].content.replaceAll(
        /<[^<>]*>/g,
        ""
      )}`
    );
    ...
    if (answerA != undefined)
      ...
      $answerParagraph.append(
        `<strong>A. </strong> <img src="${questionsAndAnswer.question.answers[0].text[0].url}"><br>`
      );
    if (answerB != undefined)
      $answerParagraph.append(`<strong>B. </strong> ${answerB}<br>`);
    else
      $answerParagraph.append(
        `<strong>B. </strong> <img src="${questionsAndAnswer.question.answers[1].text[0].url}"><br>`
      );
    if (answerC != undefined)
      $answerParagraph.append(`<strong>C. </strong> ${answerC}<br>`);
    else
      $answerParagraph.append(
        `<strong>C. </strong> <img src="${questionsAndAnswer.question.answers[2].text[0].url}"><br>`
      );
    if (answerD != undefined)
      $answerParagraph.append(`<strong>D. </strong> ${answerD}<br>`);
    else
      $answerParagraph.append(
        `<strong>D. </strong> <img src="${questionsAndAnswer.question.answers[3].text[0].url}"><br>`
      );
  });
...
async function renderHTMLtoLatex(HTMLdata: any, outputName: string) {
  ...
  let metadataFile = fileURLToPath(
    new URL("../output/metadata.yaml", import.meta.url)
  );
  let outputFile = fileURLToPath(
